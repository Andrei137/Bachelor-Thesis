...
queryGreek greekMap string =
    lookupMay string greekMap `bind` \xs ->
    tailMay xs `bind` \xsTail ->
    maximumMay xsTail `bind` \dividend ->
    headMay xs `bind` \divisor ->
...
addSalaries salariesMap name1 name2 =
    lookupMay name1 salariesMap `bind` \salary1 ->
    lookupMay name2 salariesMap `bind` \salary2 ->
